...
#[cfg(not(any(test, feature = "date-sim-clock")))]
pub fn now_utc() -> UtcDateTime {
    UtcDateTime::now()
}
...
impl WithOffsetParser {
    ...
    pub fn parse(self, input: &str) -> Result<OffsetDateTime, FmtErr> {
        OffsetDateTime::parse(input, self.description()).map_err(|error| {
            BaseFmtErr::DateError.to_err(serde_json::json!({
                "cause": format!("parse datetime with offset failed: {error}, input={input}")
            }))
        })
    }
}
...
pub trait DateTimeFormattable {
    ...
    fn format_ext_rfc3339(&self) -> Result<String, FmtErr> {
        self.format_with(&Rfc3339).map_err(|e| {
            BaseFmtErr::DateError.to_err(serde_json::json!({
                "cause": format!("format RFC 3339 datetime failed: {e}")
            }))
        })
    }

    fn format_ext(&self, formatter: DateTimeFormatter) -> Result<String, FmtErr> {
...
impl DateTimeFormatter {
    pub fn format<T: HygieaDateTimeExt>(self, datetime: &T) -> Result<String, FmtErr> {
...
pub trait HygieaDateTimeExt: Sized + DateTimeFormattable {
    /// 从 Unix 毫秒时间戳创建日期时间。
    fn from_millis(ms: i64) -> Result<Self, FmtErr>;
    /// 从 Unix 秒时间戳创建日期时间。
    fn from_secs(s: i64) -> Result<Self, FmtErr>;

    /// 解析携带 offset 的 RFC 3339；时区/offset 语义由实现类型决定。
    fn parse_ext_rfc3339(s: &str) -> Result<Self, FmtErr>;
    /// 按实现类型自身的时区/offset 语义输出 RFC 3339。
    fn format_ext_rfc3339(&self) -> Result<String, FmtErr> {
        DateTimeFormattable::format_ext_rfc3339(self)
    }
    /// 按指定格式输出日期时间。
    fn format_ext(&self, formatter: impl Into<DateTimeFormatter>) -> Result<String, FmtErr> {
        DateTimeFormattable::format_ext(self, formatter.into())
    }

    /// 在绝对时间线上平移指定时长。
    fn shift(&self, duration: Duration) -> Result<Self, FmtErr>;
}
...
pub trait HygieaUtcDateTimeExt: HygieaDateTimeExt {
    ...
    /// 返回当前日期的开始。
    fn start_of_day(&self) -> Result<Self, FmtErr>;
    /// 返回当前日期的结束。
    fn end_of_day(&self) -> Result<Self, FmtErr>;

    // ---- 周边界 ----------------------------------------------------------------

    /// 返回当前周的开始，周一为一周第一天。
    fn start_of_week(&self) -> Result<Self, FmtErr>;
    /// 返回当前周的结束，周日为一周最后一天。
    fn end_of_week(&self) -> Result<Self, FmtErr>;

    // ---- 月边界 ----------------------------------------------------------------

    /// 返回当前月的开始。
    fn start_of_month(&self) -> Result<Self, FmtErr>;
    /// 返回当前月的结束。
    fn end_of_month(&self) -> Result<Self, FmtErr>;

    // ---- 年边界 ----------------------------------------------------------------

    /// 返回当前年的开始。
    fn start_of_year(&self) -> Result<Self, FmtErr>;
    /// 返回当前年的结束。
    fn end_of_year(&self) -> Result<Self, FmtErr>;
}

impl HygieaDateTimeExt for UtcDateTime {
    fn from_millis(ms: i64) -> Result<Self, FmtErr> {
        UtcDateTime::from_unix_timestamp_nanos(ms as i128 * 1_000_000).map_err(|e| {
            BaseFmtErr::DateError.to_err(serde_json::json!({
                "cause": format!("timestamp millis out of range: {ms}, {e}")
            }))
        })
    }

    fn from_secs(s: i64) -> Result<Self, FmtErr> {
        UtcDateTime::from_unix_timestamp(s).map_err(|e| {
            BaseFmtErr::DateError.to_err(serde_json::json!({
                "cause": format!("timestamp secs out of range: {s}, {e}")
            }))
        })
    }

    fn parse_ext_rfc3339(s: &str) -> Result<Self, FmtErr> {
        UtcDateTime::parse(s, &Rfc3339).map_err(|e| {
            BaseFmtErr::DateError.to_err(serde_json::json!({
                "cause": format!("parse RFC 3339 UTC datetime failed: {e}, input={s}")
            }))
        })
    }
...
impl HygieaUtcDateTimeExt for UtcDateTime {
    ...
    fn start_of_week(&self) -> Result<Self, FmtErr> {
        ...
        midnight.checked_sub(Duration::days(days)).ok_or_else(|| {
            BaseFmtErr::DateError.to_err(serde_json::json!({
                "cause": format!("start_of_week out of range: datetime={self}")
            }))
        })
    }
    ...
    fn end_of_week(&self) -> Result<Self, FmtErr> {
        self.start_of_week()?
            ...
            .ok_or_else(|| {
                BaseFmtErr::DateError.to_err(serde_json::json!({
                    "cause": format!("end_of_week out of range: datetime={self}")
                }))
            })
    }

    fn start_of_month(&self) -> Result<Self, FmtErr> {
        let date = self.date().replace_day(1).map_err(|e| {
            BaseFmtErr::DateError.to_err(serde_json::json!({
    ...
    fn end_of_month(&self) -> Result<Self, FmtErr> {
        let next_month = self
            ...
            .ok_or_else(|| {
                BaseFmtErr::DateError.to_err(serde_json::json!({
                    "cause": format!("end_of_month out of range: datetime={self}")
                }))
            })?;
        let next_month = UtcDateTime::new(
            next_month.date().replace_day(1).map_err(|e| {
                BaseFmtErr::DateError.to_err(serde_json::json!({
                    "cause": format!("end_of_month failed: {e}")
                }))
            })?,
            Time::MIDNIGHT,
        );
    ...
    fn end_of_year(&self) -> Result<Self, FmtErr> {
        let next_year = self.year().checked_add(1).ok_or_else(|| {
            BaseFmtErr::DateError.to_err(serde_json::json!({
                "cause": format!("end_of_year out of range: datetime={self}")
            }))
        })?;
        let date = self
            .start_of_year()?
            ...
            .map_err(|e| {
                BaseFmtErr::DateError.to_err(serde_json::json!({
                    "cause": format!("end_of_year failed: {e}")
                }))
            })?;
...
impl HygieaDateTimeExt for OffsetDateTime {
    ...
    fn parse_ext_rfc3339(s: &str) -> Result<OffsetDateTime, FmtErr> {
        OffsetDateTime::parse(s, &Rfc3339).map_err(|e| {
            BaseFmtErr::DateError.to_err(serde_json::json!({
                "cause": format!("parse RFC 3339 datetime failed: {e}, input={s}")
            }))
        })
    }

    fn shift(&self, duration: Duration) -> Result<OffsetDateTime, FmtErr> {
        self.checked_add(duration).ok_or_else(|| {
            BaseFmtErr::DateError.to_err(serde_json::json!({
                "cause": format!(
                    "shift datetime out of range: datetime={self}, duration={duration}"
...
pub(super) mod iana {
    ...
    impl WithoutOffsetParser {
        ...
        pub fn parse(self, input: &str) -> Result<OffsetResult<OffsetDateTime>, FmtErr> {
            let datetime =
                PrimitiveDateTime::parse(input, self.description()).map_err(|error| {
                    BaseFmtErr::DateError.to_err(serde_json::json!({
                        "cause": format!(
                            "parse datetime without offset failed: {error}, input={input}"
                        )
                    }))
                })?;
    ...
    pub trait HygieaOffsetDateTimeExt: HygieaDateTimeExt {
        ...
        ) -> Result<OffsetResult<OffsetDateTime>, FmtErr>;
        /// 转换到系统 IANA 时区在该时间点使用的 offset，再按指定格式输出。
        fn format_ext_local(&self, formatter: DateTimeFormatter) -> Result<String, FmtErr>;

        // ---- 时间平移 ------------------------------------------------------------

        /// 在绝对时间线上平移指定时长，再转换到系统 IANA 时区。
        fn shift_local(&self, duration: Duration) -> Result<OffsetDateTime, FmtErr>;

        // ---- 日边界 ----------------------------------------------------------------

        /// 按系统 IANA 时区返回当前本地日期的开始。
        fn start_of_day_local(&self) -> Result<OffsetResult<OffsetDateTime>, FmtErr>;
        /// 按系统 IANA 时区返回当前本地日期的结束。
        fn end_of_day_local(&self) -> Result<OffsetResult<OffsetDateTime>, FmtErr>;

        // ---- 周边界 ----------------------------------------------------------------

        /// 按系统 IANA 时区返回当前本地周的开始，周一为一周第一天。
        fn start_of_week_local(&self) -> Result<OffsetResult<OffsetDateTime>, FmtErr>;
        /// 按系统 IANA 时区返回当前本地周的结束，周日为一周最后一天。
        fn end_of_week_local(&self) -> Result<OffsetResult<OffsetDateTime>, FmtErr>;

        // ---- 月边界 ----------------------------------------------------------------

        /// 按系统 IANA 时区返回当前本地月的开始。
        fn start_of_month_local(&self) -> Result<OffsetResult<OffsetDateTime>, FmtErr>;
        /// 按系统 IANA 时区返回当前本地月的结束。
        fn end_of_month_local(&self) -> Result<OffsetResult<OffsetDateTime>, FmtErr>;

        // ---- 年边界 ----------------------------------------------------------------

        /// 按系统 IANA 时区返回当前本地年的开始。
        fn start_of_year_local(&self) -> Result<OffsetResult<OffsetDateTime>, FmtErr>;
        /// 按系统 IANA 时区返回当前本地年的结束。
        fn end_of_year_local(&self) -> Result<OffsetResult<OffsetDateTime>, FmtErr>;
    }
    ...
    impl HygieaOffsetDateTimeExt for OffsetDateTime {
        ...
        fn shift_local(&self, duration: Duration) -> Result<OffsetDateTime, FmtErr> {
            ...
            .ok_or_else(|| BaseFmtErr::DateError.to_err(serde_json::json!({
                "cause": format!("shift datetime out of range: datetime={self}, duration={duration}")
            })))
        }
        ...
        fn start_of_week_local(&self) -> Result<OffsetResult<OffsetDateTime>, FmtErr> {
            ...
            let date = local
                ...
                .ok_or_else(|| {
                    BaseFmtErr::DateError.to_err(serde_json::json!({
                        "cause": format!("start_of_week_local out of range: datetime={self}")
                    }))
                })?;
        ...
        fn end_of_week_local(&self) -> Result<OffsetResult<OffsetDateTime>, FmtErr> {
            ...
            let date = local
                ...
                .ok_or_else(|| {
                    BaseFmtErr::DateError.to_err(serde_json::json!({
                        "cause": format!("end_of_week_local out of range: datetime={self}")
                    }))
                })?;
        ...
        fn start_of_month_local(&self) -> Result<OffsetResult<OffsetDateTime>, FmtErr> {
            let local = self.to_timezone(local_timezone());
            let date = local.date().replace_day(1).map_err(|e| {
                BaseFmtErr::DateError.to_err(serde_json::json!({
                    "cause": format!("start_of_month_local failed: {e}")
                }))
            })?;
            Ok(resolve_local(PrimitiveDateTime::new(date, Time::MIDNIGHT)))
        }
        ...
        fn end_of_month_local(&self) -> Result<OffsetResult<OffsetDateTime>, FmtErr> {
            ...
            let first = PrimitiveDateTime::new(
                local.date().replace_day(1).map_err(|e| {
                    BaseFmtErr::DateError.to_err(serde_json::json!({
                        "cause": format!("end_of_month_local failed: {e}")
                    }))
                })?,
                Time::MIDNIGHT,
            );
            let next = first
                ...
                .ok_or_else(|| {
                    BaseFmtErr::DateError.to_err(serde_json::json!({
                        "cause": format!("end_of_month_local out of range: datetime={self}")
                    }))
                })?;
        ...
        fn start_of_year_local(&self) -> Result<OffsetResult<OffsetDateTime>, FmtErr> {
            ...
            let date = local
                ...
                .map_err(|e| {
                    BaseFmtErr::DateError.to_err(serde_json::json!({
                        "cause": format!("start_of_year_local failed: {e}")
                    }))
                })?;
            Ok(resolve_local(PrimitiveDateTime::new(date, Time::MIDNIGHT)))
        }
        ...
        fn end_of_year_local(&self) -> Result<OffsetResult<OffsetDateTime>, FmtErr> {
            ...
            let next_year = local.year().checked_add(1).ok_or_else(|| {
                BaseFmtErr::DateError.to_err(serde_json::json!({
                    "cause": format!("end_of_year_local out of range: datetime={self}")
                }))
            })?;
            let date = local
                .date()
                ...
                .map_err(|e| {
                    BaseFmtErr::DateError.to_err(serde_json::json!({
                        "cause": format!("end_of_year_local failed: {e}")
                    }))
                })?;
    ...
    mod tests {
        ...
        fn unique(result: Result<OffsetResult<OffsetDateTime>, FmtErr>) -> OffsetDateTime {
            match result {
...
// ---- chrono 互转桥（feature = "date-chrono"）-----------------------------------
...
mod chrono_bridge {
    ...
    pub trait DateTimeUtcExt {
        fn to_utc_datetime(&self) -> Result<UtcDateTime, FmtErr>;
        fn from_utc_datetime(datetime: UtcDateTime) -> Result<DateTime<Utc>, FmtErr>;
    }
    ...
    impl DateTimeUtcExt for DateTime<Utc> {
        fn to_utc_datetime(&self) -> Result<UtcDateTime, FmtErr> {
            ...
            UtcDateTime::from_unix_timestamp_nanos(nanos).map_err(|e| {
                BaseFmtErr::DateError.to_err(serde_json::json!({
                    "cause": format!("to_utc_datetime out of range: {e}")
                }))
            })
        }
        ...
        fn from_utc_datetime(datetime: UtcDateTime) -> Result<DateTime<Utc>, FmtErr> {
            DateTime::from_timestamp(datetime.unix_timestamp(), datetime.nanosecond()).ok_or_else(
                || {
                    BaseFmtErr::DateError.to_err(serde_json::json!({
                        "cause": "from_utc_datetime: timestamp out of range"
                    }))
                },
            )
        }
